...
public class Driver {
    ...
    public void getInputs() {
        System.out.println("Enter the total no of floors in the Skyscraper");
    ...
    public void calculateAndPrintOrderOfConstruction(Integer[] totalFloorSize) {
        System.out.println("The order of Skyscraper Building construction is as follows");
        ...
        //Loop over the unsorted data and pop all Stack elements 
        for (int i = 0; i < totalFloorSize.length; i++) {
            if (totalFloorSize[i] == totalFloorSizeSorted[j]) {
                if (i == totalFloorSize.length - 1) j = i;
                else j = i + 1;

                // Print the current floor as well as entire Stack till it's empty and entry is greater than next element in sorted list.
